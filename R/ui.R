...
ui.top_row <- function(suffix) {
  return(fluidRow(
    id = "search-row",
    column(1),
    column(width = 2, selectInput(paste0("mode", suffix),
      label = "Input",
      choices = c(
        "Metabolite by HMDB identifier", "Metabolites by name", "Immune process by name", "Immune process by name (without proteins)",
    ...
    column(width = 4, selectizeInput(paste0("filter", suffix),
      label = "Data", choices = NULL, multiple = T,
...
ui <- function() {
  dashboardPage(
    ...
    dashboardSidebar(
      width = 300, useShinyjs(), use_waiter(),
        ...
        div(
          ...
          tabItems(
            tabItem(
              tabName = "home", div(
                id = "dashboardTab",
                fluidRow(div(class = "dashboard_head1"),
                  align = "center", column(width = 1),
                  column(
                    width = 8, hr(), h1("The Immunometabolic Atlas"),
                ...
                ui.top_row(""),
                fluidRow(div(class = "dashboard_head2"),
                  align = "center", column(width = 1),
                  infoBox(
                    title = strong("Network"), width = 2, icon = icon("project-diagram"),
                    subtitle = "Build a network-structure of your data.", color = "blue"
                  ),
                  infoBox(
                    title = strong("Heatmap"), width = 2, icon = icon("th"), color = "green",
                    subtitle = "View distances between targets using a heatmap."
                  ),
                  infoBox(
                    title = strong("Statistics"), width = 2, icon = icon("chart-bar"), color = "red",
                    subtitle = "Map Gene Ontologies to metabolites."
                  ),
                  infoBox(
                    title = strong("Data Table"), width = 2, icon = icon("table"), color = "orange",
            ...
            tabItem(
              tabName = "network", header_row("Results", ""),
              ui.top_row("Graph"),
